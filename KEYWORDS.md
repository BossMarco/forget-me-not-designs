# Forget Me Not Flowers — Target Keyword List

Domain: https://www.forgetmenotrgv.com — launched 2026-06-07.
Full machine list (600 tracked in SerpBear): `Projects/seo-stack/forgetmenot-keywords.tsv`

**Reality check:** brand-new domain. Indexing takes days–weeks; ranking for
competitive local terms ("flower shop mcallen") takes 2–6+ months and depends
heavily on a claimed/optimized **Google Business Profile** (the map pack ranks
above organic for these searches). Check progress with `site:forgetmenotrgv.com`
in Google and in Search Console → Pages.

`{city}` = each RGV city we serve (McAllen, Edinburg, Pharr, Mission, Weslaco,
Harlingen, Brownsville, South Padre Island, San Juan, Alamo, Donna, Mercedes,
Hidalgo, Palmview, Alton, Rio Grande City, Los Fresnos, San Benito, Port Isabel,
Raymondville, + ~28 smaller towns).

---

## 1. Core commercial — highest value (per city)

English:

- florist {city}
- flower shop {city}
- flower delivery {city}
- same day flower delivery {city}
- flower shop near me / florist near me / flower delivery near me

Spanish:

- floreria {city}
- floreria en {city}
- envio de flores {city}
- flores a domicilio {city}
- floreria cerca de mi

## 2. Everyday occasions — the bread & butter (per city)

English:

- birthday flowers {city}
- anniversary flowers {city}
- funeral flowers {city}
- sympathy flowers {city}
- get well flowers {city}
- roses delivery {city}
- congratulations flowers {city}
- just because flowers {city}

Spanish:

- flores para cumpleaños {city}
- flores para aniversario {city}
- flores para funeral {city}
- ramos de rosas {city}
- rosas a domicilio {city}
- arreglos florales {city}

## 3. Big-ticket events (per city)

English:

- wedding florist {city}
- wedding flowers {city}
- quinceanera flowers {city}
- prom flowers {city} / prom corsages {city}
- baby shower flowers {city}
- gender reveal flowers {city}

Spanish:

- flores para boda {city}
- flores para quinceañera {city}
- centros de mesa {city}
- flores para baby shower {city}
- flores para graduacion {city}

## 4. Seasonal (per city + region) — go big near the date

- valentines day flowers {city} / flores para san valentin {city}
- mothers day flowers {city} / flores para el dia de las madres {city}

## 5. Region / Valley-wide

English:

- florist rgv
- florist rio grande valley
- flower delivery rgv
- flower delivery rio grande valley
- flower shop rgv
- same day flower delivery rgv
- wedding florist rgv
- quinceanera flowers rgv
- funeral flowers rgv

Spanish:

- floreria rgv
- floreria valle de texas
- envio de flores rgv
- flores a domicilio rgv
- flores a domicilio valle de texas
- flores para quinceañera rgv
- flores para funeral rgv
- arreglos florales rgv

## 6. Brand

- forget me not flowers
- forget me not flowers and gifts
- forget me not flowers mcallen
- forget me not flowers rgv
- forget me not florist

---

## How each keyword maps to a page

- `florist/flower shop/flower delivery {city}` → city hub `/flower-delivery/{city}`
- `{occasion} flowers {city}` → matrix page `/flower-delivery/{city}/{occasion}`
- `{occasion} flowers rgv` → occasion hub `/occasions/{occasion}`
- Brand terms → homepage `/`

## Priority order for ranking effort

1. Claim + optimize Google Business Profile (unlocks the map pack — biggest lever).
2. Brand terms (easiest wins; should rank in days–weeks once indexed).
3. Tier-1 city core terms (McAllen/Pharr/Edinburg/etc. florist + flower delivery).
4. Everyday occasions in tier-1 cities (birthday/anniversary/funeral).
5. Long-tail city × occasion (lower competition, steady traffic).
